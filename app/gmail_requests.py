...
def send_approved_request(db: Session, request: GdprRequest, sender: GmailSender, settings: Settings) -> Communication:
    ...
    if method == "portal":
        raise ValueError("Portal requests must not be sent by email")
    recipient = (resolution.dpo_contact if resolution else None) or request.company.dpo_contact or request.company.privacy_email
    if not recipient or "@" not in recipient or method not in {"email", "unknown", None}:
    ...
    communication = Communication(
        gdpr_request_id=request.id, direction="OUTBOUND", channel="email", subject=request.subject,
        body=request.body_text, sender=settings.privacy_user_preferred_email, sent_at=now,
        gmail_message_id=str(result.get("id", "")), gmail_thread_id=result.get("threadId"),
        rfc822_path=str(path), content_hash=sha256(raw or request.body_text.encode()).hexdigest(),
    )
    ...
    db.add(CaseEvent(case_id=privacy_case.id, event_type="email_sent", from_status="APPROVED", to_status="SENT", note=f"Gmail message {result.get('id', '')}"))
    db.commit()
